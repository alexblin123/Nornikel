...
# --- НАСТРОЙКА СТРАНИЦЫ ---
st.set_page_config(page_title="НОРНИКЕЛЬ | Автоматизация анализа шлифов", layout="wide")
...
if "markup_session_key" not in st.session_state:
    st.session_state.markup_session_key = "init"
if "current_verdict" not in st.session_state:
    st.session_state.current_verdict = "Рядовой"
...
# Загрузчик файлов всегда активен наверху, чтобы файл не стирался из памяти
...
if uploaded_file is not None:
    ...
    # 100% гарантия, что файл есть на диске для CV2
    with open(temp_path, "wb") as f:
        f.write(image_bytes)

    # ==============================================================
    # РЕЖИМ 1: РУЧНАЯ РАЗМЕТКА (Включается по кнопке из Режима 2)
    # ==============================================================
    if st.session_state.show_markup:
        st.markdown("#### 🖍️ Модуль ручной коррекции фаз (Active Learning)")
        st.markdown(
            "Скорректируйте границы фаз. Данные автоматически экспортируются в JSON-пакет по регламенту Норникеля.")
        st.divider()
        ...
        with modal_left:
            ...
            st.markdown("<div style='font-weight: 600; font-size: 14px;'>Минеральная фаза:</div>",
                        unsafe_allow_html=True)
            ...
            st.markdown("<hr>", unsafe_allow_html=True)
            st.markdown("<div style='font-weight: 600; font-size: 14px;'>Экспертный вердикт:</div>",
                        unsafe_allow_html=True)
        ...
        with modal_right:
            # Читаем картинку, подгоняем размеры и передаем в canvas напрямую
            bg_img = Image.open(temp_path).convert("RGB")
            orig_w, orig_h = bg_img.size

            canvas_width = 750
            canvas_height = int(orig_h * (canvas_width / float(orig_w)))
            bg_img_resized = bg_img.resize((canvas_width, canvas_height), Image.Resampling.LANCZOS)

            # 💡 ГЛАВНЫЙ ФИКС: Уникальный динамический ключ обходит баг Streamlit Cloud!
            canvas_result = st_canvas(
                ...
                key=f"expert_canvas_{st.session_state.markup_session_key}"
            )
        ...
        if submit_clicked and canvas_result is not None:
            if canvas_result.image_data is not None:
                ...
                json_path = os.path.join("active_learning_dataset", f"markup_{uploaded_file.name}.json")
                with open(json_path, "w", encoding="utf-8") as jf:
                    json.dump(export_payload, jf, indent=4, ensure_ascii=False)

                cv2.imwrite(os.path.join("active_learning_dataset", f"mask_ordinary_{uploaded_file.name}.png"),
                            mask_ordinary_orig)
                cv2.imwrite(os.path.join("active_learning_dataset", f"mask_thin_{uploaded_file.name}.png"),
                            mask_thin_orig)
                cv2.imwrite(os.path.join("active_learning_dataset", f"mask_talc_{uploaded_file.name}.png"),
                            mask_talc_orig)
    ...
    # ==============================================================
    # РЕЖИМ 2: ОСНОВНОЙ ДАШБОРД (Анализ ML)
    # ==============================================================
    else:
        ...
        with main_col2:
            ...
            if st.button("🖍️ Включить режим ручной разметки зон", use_container_width=True):
                ...
                # 💡 Генерируем уникальный ключ, чтобы холст загрузился как "чистый лист"
                st.session_state.markup_session_key = str(datetime.datetime.now().timestamp())
